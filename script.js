/*
  ...
  - Contador regresivo hasta el 10 de abril de 2026 (hora local del usuario)
  - Animaciones suaves (nubes, brillos, corazones/estrellas), parallax y reveal al hacer scroll
*/

(() => {
  const prefersReducedMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches ?? false;
  const TARGET_LOCAL = new Date(2026, 3, 10, 0, 0, 0); // 10 de abril 2026 00:00:00 (hora local)
  const GA_ANCHOR_LOCAL = new Date(2026, 3, 4, 0, 0, 0); // 4 de abril 2026 00:00:00 (hora local)
  const GA_ANCHOR_TOTAL_DAYS = 36 * 7 + 6;
  const BGM_SRC = "./audio/Tarzán - En Mi Corazón Vivirás (By_ Phil Collins) (Letra).mp3";
  ...
  const ULTRASOUNDS = [
    ...
    "ultrasonidos/BRENDA HDZ ALVAREZ 2.pdf",
  ];
  ...
  const els = {
    ...
    gaValue: document.getElementById("gaValue"),
  ...
  const startOfDayLocal = (d) => new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0);
  ...
  const updateGestationalAge = () => {
    if (!els.gaValue) return;

    const now = new Date();
    const diffMs = startOfDayLocal(now).getTime() - startOfDayLocal(GA_ANCHOR_LOCAL).getTime();
    const diffDays = Number.isFinite(diffMs) ? Math.round(diffMs / 86400000) : 0;

    const totalDays = Math.max(0, GA_ANCHOR_TOTAL_DAYS + diffDays);
    const weeks = Math.floor(totalDays / 7);
    const days = totalDays % 7;

    els.gaValue.textContent = `${weeks} semanas y ${days} días`;
  };

  const setMusicUi = (isPlaying) => {
    if (els.musicToggle) els.musicToggle.classList.toggle("is-playing", isPlaying);
    if (els.musicToggle) els.musicToggle.setAttribute("aria-label", isPlaying ? "Pausar música" : "Reproducir música");
    if (els.musicLabel) els.musicLabel.textContent = isPlaying ? "Pausar" : "Música";
  };
  ...
  const tryPlayMusic = async () => {
    ...
    if (!audio.src) audio.src = encodeURI(BGM_SRC);
    audio.preload = "auto";
  ...
  const initMusic = () => {
    const audio = els.bgm;
    if (!audio || !els.musicToggle) return;
    ...
    els.musicToggle.addEventListener("click", onToggle);
  ...
  const renderGallery = () => {
    ...
    ULTRASOUNDS.forEach((path, index) => {
      ...
      thumb.innerHTML = `
        <div class="thumb__frame" aria-hidden="true">
          <img class="thumb__img" src="./imgs/descarga%20(4).png" alt="" aria-hidden="true" loading="lazy" decoding="async" />
        </div>
  ...
  const init = () => {
    ...
    window.setInterval(updateGestationalAge, 60 * 60 * 1000);
